...
class ApiSettingController extends Controller
{
    ...
    public function show(): JsonResponse
    {
        ...
        return response()->json([
            'data' => [
                ...
                'key_issued_at'   => $settings->key_issued_at?->toISOString(),
                'key_masked'      => $settings->key_last_four ? 'cwk_live_••••••••••••' . $settings->key_last_four : null,
                'allowed_sources' => $settings->allowedSources->map(fn($source) => [
                    'id'    => $source->id,
                    'value' => $source->value,
                    'memo'  => $source->memo ?? '',
                    'kind'  => str_contains($source->value, '/') ? 'cidr' : 'ip'
                ]),
            ]
    ...
    public function update(Request $request): JsonResponse
    {
        $data = $request->validate([
            ...
            'allowed_sources'         => ['present', 'array', 'max:100'],
            'allowed_sources.*'       => ['array:value,memo'],
            'allowed_sources.*.value' => ['required', 'string', 'max:49'],
            'allowed_sources.*.memo'  => ['nullable', 'string', 'max:1000'],
        ]);

        if ($data['enabled'] && count($data['allowed_sources']) === 0) {
            throw ValidationException::withMessages([
                'allowed_sources' => 'APIを有効にするには許可IP/CIDRを1件以上登録してください。'
            ]);
        }
        ...
        foreach ($data['allowed_sources'] as $index => &$source) {
            ...
            if ($value === null || isset($seen[$value])) {
                throw ValidationException::withMessages([
                    "allowed_sources.$index.value" => '正しいIP/CIDRを重複しないように入力してください。'
                ]);
            }
            $source['value'] = $value;
            $seen[$value] = true;
